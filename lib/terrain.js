...
(function() {
  ...
  function diamond(matrix, depth, smoothness) {
    ...
    var numSegs = Math.pow(2, depth - 1);
  ...
  function square(matrix, depth, smoothness) {
    ...
    for (var i = 0; i < numSegs; ++i) {
      for (var j = 0; j < numSegs; ++j) {
        ...
        var vi = [(x + span / 2 * 3) % matrix.length, y + span / 2]; // right of h
        var vj = [(x - span / 2 + matrix.length) % matrix.length, y + span / 2]; // left of f
        var vq = [x + span / 2, (y + span / 2 * 3) % matrix.length]; // under l
        var vv = [x + span / 2, (y - span / 2 + matrix.length) % matrix.length]; // above b
